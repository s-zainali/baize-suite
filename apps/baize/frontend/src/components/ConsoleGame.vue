<template>
    <div :class="isDisplay ? 'w-[101px] h-[195px]' : ''">
        <div class="w-[202px] relative flex-none px-4" :class="isDisplay ? 'scale-50 origin-top-left' : 'mt-12'">
            ...
            <div class="flex flex-col items-center w-full relative z-10 group">
                ...
                <div class="relative w-full h-[390px] rounded-2xl border-2 shadow-xl transition-all duration-500 bg-slate-950 flex flex-col items-center px-3 pb-8 overflow-hidden"
                    ...
                    <PlayerChips v-if="roster.length && playersModal" :players="roster" @close="playersModal = false"
                        @remove="removePlayer" />
                    <div v-if="locked"
                    ...
                    <div class="w-full mt-2 mb-2 relative z-5">
                        ...
                        <div v-if="!isDisplay"
                            class="absolute -inset-2 rounded-xl blur-lg transition-opacity duration-500 pointer-events-none"
                            :class="[
                                props.table.isActive ? 'opacity-100' : 'opacity-0',
                                isPlaystation ? 'bg-blue-500/25' : isPc ? 'bg-purple-500/25' : 'bg-green-700/25'
                            ]"></div>

                        <div class="relative w-full aspect-[16/10] rounded-lg border-[3px] bg-slate-900 overflow-hidden"
                            :class="!bookedIn() || table.isActive || !showBookingStatus
                                ? 'border-slate-700'
                                : bookedIn() > 60
                                    ? 'border-emerald-400'
                                    : bookedIn() > 30
                            ...
                            <div class="absolute inset-0 flex flex-col items-center justify-center overflow-hidden px-1 transition-all duration-500 gap-1"
                                :class="props.table.isActive
                                    ...
                                    : !bookedIn() || !showBookingStatus
                                        ? 'bg-slate-950'
                                        : bookedIn() > 60
                                            ? 'text-emerald-100 bg-emerald-500/50'
                                            : bookedIn() < 30
                                                ? 'text-red-100 bg-red-500/50'
                                                : 'text-amber-100 bg-amber-400/50'">

                                <div class="flex flex-col items-center justify-center mt-2"
                                    v-if="props.table.isActive && !isDisplay">
                                    <span class="text-2xl font-mono font-bold tracking-wider text-slate-200" :class="isPlaystation
                                        ? 'drop-shadow-[0_0_6px_rgba(96,165,250,0.6)]'
                                        : isPc
                                            ? 'drop-shadow-[0_0_6px_rgba(192,132,252,0.6)]'
                                            : 'drop-shadow-[0_0_6px_rgba(74,222,128,0.6)]'">
                                        {{ formattedTime }}
                                    </span>
                                    <span class="text-[8px] font-bold uppercase tracking-wide mt-0.5"
                                        :class="isPlaystation ? 'text-blue-100/80' : isPc ? 'text-purple-100/80' : 'text-green-100/80'">
                                        Playing
                                    </span>
                                </div>
                                <template v-else>
                                    ...
                                    <div v-if="bookedIn() && showBookingStatus && !table.isActive && !table.resumable"
                                        class="uppercase text-[10px] font-bold flex flex-row items-center justify-center rounded-xl gap-2">
                                        <span class="h-fit" :class="isDisplay ? 'text-4xl' : 'text-2xl'">!</span>
                                    ...
                                    <div v-if="canResume" class="text-center">
                                        <span class="block text-sm font-black font-mono text-amber-300">Rs {{
                                            pendingTotal }}</span>
                                        <span
                                            class="block text-[8px] font-bold uppercase tracking-wide text-amber-400/80">Paused</span>
                                    </div>
                                </template>
                                <PlayerIndicator v-if="!isDisplay && roster.length" :roster="roster"
                                    @view="playersModal = true" />
                            </div>
                    ...
                    <!-- KBM Cables -->
                    <div v-if="table.type === 'pc'" class="absolute  w-29.5 h-4 z-10 "
                    ...
                    <!-- Player input -->
                    <div v-if="!isDisplay" class="w-full mt-2">
                        <PlayerPicker v-if="!canResume && !props.table.isActive" v-model="roster" :chips="false"
                            @link="linking = true" />
                    </div>
...
// Console / PC Type Normalization & Categorization
...
const playerNames = computed(() => {
    const rosterList = (props.table.players || []).map((p) => p.name).filter(Boolean)
    if (rosterList.length) return rosterList
